...
void Board::set_tile(int rank, int file, Piece piece) {
	tiles[rank][file].set_piece(piece);
}

void Board::set_tile(int rank, int file, PieceType type, PieceColor color) {
	tiles[rank][file].set_piece(Piece(type, color));
}

Tile Board::get_tile(int rank, int file) {
	if (rank < 0 || rank >= (int) this->tiles.size() || file < 0 || file >= (int) this->tiles.at(0).size()) {
		return (Tile(-1, -1));
	}
